...
from rf_tools.utils import get_barcode_kits, get_device, write_toml_file
...
def setup_caller_settings(adventure: bool) -> dict:
    """
    ...
    quests = [
        (
            "address",
            {True: f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} What is the address it lives at? Mine, for example, lives at localhost. ",
             False: f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} Please provide guppy address - default localhost. "}[adventure],
            "localhost",
        ),
        (
            "port",
            {True: f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} What port should I call at? usually Guppy instances prefer 5555. ",
             False: f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} What port is your Guppy instant running at? Default 5555. "}[adventure],
            "5555",
        ),
    ]
    dicty = {}
...
def barcoding_adventure(args, version) -> None:
    ...
    logger.warning(
        "Hit enter to provide a default values for any fields with one provided in the prompt.\n"
    )

    choices = {"deplete", "enrich"}
    type_of_exp = prompt_user(
        {
            False: f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} Do you wish to [bold red]deplete[/] or [bold green]enrich[/]?",
            True:  f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} Doth the desire to [bold red]deplete[/] or [bold green]enrich[/]?",
        }[adventure],
    ...
    caller_settings_dict = setup_caller_settings(adventure)

    pat = re.compile(r"(\d{2},*$)|(\d{2},)+\d{2}$|(\d{2},)+\d{2},$")
    barcode_string = prompt_user(
        f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} Comma seperated, two digit barcodes to {type_of_exp} - eg. 02,05,40,20,19 ",
        [console.input],
    ...
    barcode_kit = prompt_user(
        {
            True: f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} Which mighty barcode kit did you yield?",
            False: f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} Please input barcode kit name - default EXP-NBD196",
        }[adventure],
        [console.input],
    ...
    basecall_config = prompt_user(
        {
            True: f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} And which monstrous base calling config devoured your reads? (default on enter dna_r9.4.1_450bps_fast)",
            False: f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} Please provide a base calling config, default dna_r9.4.1_450bps_fast.",
        }[adventure],
        [console.input],
        f"",
    ...
    chunks_string = prompt_user(
        {
            True: f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} What is the min/max on your chunk die? Type min/max, i.e 0/4 DO NOT PUT MIN HIGHER THAN MAX",
            False: f"{datetime.datetime.now().replace(tzinfo=None, microsecond=0)} Please specify your min and max chunks for read capture: ",
        }[adventure],
        [console.input],
    ...
    reference_prompt = (
        ...
        if adventure
        else "Please provide the path to your reference. "
    )
    logger.info(reference_prompt)
